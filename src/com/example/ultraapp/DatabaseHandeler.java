...
public class DatabaseHandeler {
	 ...
	 * Setting chat text to loaded datbase text. 
	 * Updating textfile.
	 */
			...
			public void onSuccess(JSONArray messages) {
				...
				try {
					for (int i = 0; i < messages.length(); i++) {
						...
						chatHistory = MainActivity.getFormatedLine(currentLine, currentUsername, currentColor) + "<br />"+ chatHistory;
				...
				} catch (JSONException e) {message.showStatus("ERROR: DB-load 1!");}
			}
	...
	/*
	 * Uploads text to database. 
	 * Update through async client from Connector class.
